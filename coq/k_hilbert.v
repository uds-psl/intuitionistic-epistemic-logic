...
Inductive prf : list form -> form -> Prop :=
 ...
 | k Γ {p q}     : prf Γ ( K(p ⊃ q) ⊃ (K(p) ⊃ K(q)))
 | mp Γ p q      : prf Γ (p ⊃ q) -> prf Γ p -> prf Γ q
 | nec Γ p       : prf [] p -> prf Γ (K(p)) 
.
...
| ndcII A s t : ndc (s::A) t -> ndc A (Imp s t)
| ndcIE A s t : ndc A (Imp s t) -> ndc A s -> ndc A t
| ndcC A s : ndc (Not s :: A) Bot -> ndc A s
| ndcNec A s : ndc [] s -> ndc A (K s)
| ndcKD A s t: ndc A (K(s ⊃ t)) -> ndc A ((K s) ⊃ (K t)) .
...
Lemma ndc_weak A A' s :
  A <<= A' -> ndc A s -> ndc A' s.
Proof.
  enough (ndc A s -> forall A', A <<= A' -> ndc A' s).
  firstorder eauto. 
  
  intros H1. induction H1; eauto.
Qed.
...
Proof.
  ...
  - intro.  remember (p::Γ) as Γ'. dependent induction H.  
    ...
    + apply mp with (K (p0 ⊃ q0)).  apply k.  rewrite HeqΓ'. apply ax. auto.
...
Admitted.   
Theorem hilbert_nd Γ A: prf Γ A -> ndc Γ A.
Proof.
  intro. induction H; eauto.
  - repeat apply ndcII. apply ndcIE with (q ⊃ r). apply ndcII. apply ndcIE with q; eauto. apply ndcIE with p; eauto.
    apply ndcIE with p. eauto.  eauto. 
  - repeat apply ndcII.  apply ndcC.
    apply ndcIE with q.  apply ndcIE with (Not q). apply ndcII. apply ndcII. apply ndcIE with q;  apply ndcA; auto.
    all: apply ndcIE with (¬ p);  apply ndcA; auto.
Qed.

Theorem nd_hilbert Γ A: ndc Γ A -> prf Γ A.
Proof.
  intro. induction H; eauto.
  - apply deduction_thm in IHndc.  auto. 
  - apply deduction_thm in IHndc.  apply hilbert_dne in IHndc.  auto. 
Qed. *)
...
apply ndc_imp.   apply extend_does_not_derive_imp.  auto.  auto.  auto.
  ...
  Proof. 
    ...
    -   intros w. split.
        ...
        * exfalso.    apply n. apply Tdedclos.  apply negAllImoClosed in Hx.   tauto.   apply ndcIE with x1;  apply ndcA; auto.  
